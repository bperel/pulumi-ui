...
const elements = computed(() => {
  ...
  // Helper function to calculate edge offset to avoid overlapping
  const calculateEdgeOffset = (edgeIndex: number, totalEdges: number) => {
    if (totalEdges <= 1) return 0
    
    // Calculate offset based on edge position in the bundle
    const offset = (edgeIndex - (totalEdges - 1) / 2) * 10 // 10px spacing between edges
    return offset
  }

  // Helper function to get all edges between two nodes
  const getEdgesBetweenNodes = (sourceId: string, targetId: string) => {
    const edges: any[] = []
    for (const resource of stack.resources) {
      // Parent-child relationships
      if (resource.parent === sourceId && resource.urn === targetId) {
        edges.push({ type: 'parent-child', source: sourceId, target: targetId, resource })
      }
      if (resource.parent === targetId && resource.urn === sourceId) {
        edges.push({ type: 'parent-child', source: targetId, target: sourceId, resource })
      }
      
      // Dependencies
      if (resource.urn === sourceId && resource.dependencies?.includes(targetId)) {
        edges.push({ type: 'dependency', source: targetId, target: sourceId, resource })
      }
      if (resource.urn === targetId && resource.dependencies?.includes(sourceId)) {
        edges.push({ type: 'dependency', source: sourceId, target: targetId, resource })
      }
    }
    return edges
  }
  ...
  // Create edges with improved routing and bundling
  const edges: Edge[] = []
    ...
    if (resource.parent) {
      ...
      if (parentResource) {
        ...
        if (!edgeIds.has(edgeId)) {
          ...
          // Calculate edge curvature and offset
          ...
          // Calculate offset for edge bundling
          const edgesBetweenNodes = getEdgesBetweenNodes(parentResource.urn, resource.urn)
          const edgeIndex = edgesBetweenNodes.findIndex(edge => 
            edge.type === 'parent-child' && edge.source === parentResource.urn && edge.target === resource.urn
          )
          const offset = calculateEdgeOffset(edgeIndex, edgesBetweenNodes.length)
          
          // Adjust stroke width based on number of edges between nodes
          const baseStrokeWidth = isStackEdge ? 1 : (isConnected ? 3 : 2)
          const adjustedStrokeWidth = edgesBetweenNodes.length > 1 ? baseStrokeWidth - 0.5 : baseStrokeWidth
        ...
        edges.push({
          ...
          target: resource.urn,
            ...
            style: { 
              ...
              opacity: isStackEdge ? 0.6 : (selectedNodeId.value && !isConnected ? 0.3 : 0.9),
              zIndex: edgesBetweenNodes.length > 1 ? edgeIndex : 0
            },
            data: {
    ...
    for (const depId of resource.dependencies || []) {
      ...
      if (depResource) {
        ...
        if (!edgeIds.has(edgeId) && !edgeIds.has(reverseEdgeId) && !isParentChild) {
          ...
          // Calculate edge curvature and offset for dependencies
          ...
          // Determine source and target handles based on levels
          const depLevel = nodeLevels.get(depId) || 0
          const targetLevel = nodeLevels.get(resource.urn) || 0
          const sourceHandle = depLevel < targetLevel ? 'bottom' : 'top'
          const targetHandle = depLevel < targetLevel ? 'top' : 'bottom'
          
          // Calculate offset for dependency edges
          const edgesBetweenNodes = getEdgesBetweenNodes(depId, resource.urn)
          const edgeIndex = edgesBetweenNodes.findIndex(edge => 
            edge.type === 'dependency' && edge.source === depId && edge.target === resource.urn
          )
          const offset = calculateEdgeOffset(edgeIndex, edgesBetweenNodes.length)
          
          // Adjust stroke width and opacity for dependency edges
          const baseDepStrokeWidth = isStackEdge ? 1 : (isConnected ? 2 : 1)
          const adjustedDepStrokeWidth = edgesBetweenNodes.length > 1 ? baseDepStrokeWidth - 0.3 : baseDepStrokeWidth
        ...
        edges.push({
          ...
          target: resource.urn,
            ...
            style: { 
              ...
              strokeWidth: adjustedDepStrokeWidth, 
              strokeDasharray: isStackEdge ? 'none' : '5,5',
              opacity: isStackEdge ? 0.6 : (selectedNodeId.value && !isConnected ? 0.3 : 0.8),
              zIndex: edgesBetweenNodes.length > 1 ? edgeIndex : 0
            },
            data: {
              curvature: isStackEdge ? 0 : curvature,
...
const onNodeClick = (node: any) => {
  ...
  if (selectedNodeId.value === node.id) {
    ...
    emit('viewResource', node.data.resource)
  }
}
